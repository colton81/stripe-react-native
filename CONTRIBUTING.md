...
- Run `yalc publish` in `@colton81/stripe-react-native` package to publish all the files that should be published in remote NPM registry.
- Run `yalc add @colton81/stripe-react-native` in your dependent project, which will copy the current version from the store to your project's .yalc folder and inject a file:.yalc/@colton81/stripe-react-native into package.json.
